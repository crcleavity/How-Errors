...
class FiveJobSpider(scrapy.Spider):
    ...
    custom_settings = {
        "ITEM_PIPELINES":{
            ...
            'tutorial.MySQLPipline.MySQLPipeline': 300,
        },
    ...
    def parse_item(self,response):
        ...
        item['salary'] = response.xpath("/html/body/div[1]/div[2]/div[1]/div/div[1]/p[2]/text()").extract() or "面议"
        item['create_time'] = response.xpath("/html/body/div[1]/div[2]/div[1]/div/div[2]/span/text()").extract()
        item['body']  = response.xpath("/html/body/div[1]/div[2]/div[3]/div[2]/article/p/text()").extract()
        item['company_name'] = response.xpath("/html/body/div[1]/div[2]/div[2]/div/div[2]/h3/text()").extract()
        item['postion_id'] = response.url.split("/")[-1].split('.')[0]
        item['position_name'] = response.xpath("/html/body/div[1]/div[2]/div[1]/div/div[1]/p[1]/text()").extract()
        item['work_year'] = response.xpath("/html/body/div[1]/div[2]/div[1]/div/div[2]/div/span[4]/text()").extract() or "不限"
        item['educational']  = response.xpath("/html/body/div[1]/div[2]/div[1]/div/div[2]/div/span[3]/text()").extract() or "不限"
