...
class FiveJobPipeline(object):
    ...
    def __init__(self):
        # 创建一个文件
        self.file = open('huluwa.csv','w',encoding='utf-8',newline='')
        # 创建一个csv文件的对象
    ...
    def process_item(self, item, spider):
        ...
        # 写入每行数据
        self.writer.writerows((item['address'],item['salary'],item['create_time'],item['body'],item['company_name'],item['postion_id'],item['position_name'],item['work_year'],item['educational']))
        return item
